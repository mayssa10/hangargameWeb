...
class ConsoleController extends Controller
{
    ...
    {
        $console = new Console();
        $form = $this->createForm('Backend\ConsoleBundle\Form\ConsoleType', $console);
    ...
    {
        $deleteForm = $this->createDeleteForm($console);
        $editForm = $this->createForm('Backend\ConsoleBundle\Form\ConsoleType', $console);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('console_index', array('id' => $console->getId()));
        }
